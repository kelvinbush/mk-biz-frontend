...
const formSchema = z.object({
  ...
  partnershipDeed: z.string().min(1, "Partnership deed is required"),
  taxRegistration: z
    .string()
...
type PartnershipRegistrationFormProps = {
  onNext: () => void;
  businessGuid: string;
...
export function PartnershipRegistrationForm({
  ...
  onLoadingChange,
  onHelpClick,
}: PartnershipRegistrationFormProps) {
  ...
  useEffect(() => {
    ...
    const partnershipDeed = existingDocuments.find(
      (doc) => doc.docType === DocType.PartnershipDeed,
    );
    ...
    form.reset(
      {
        ...
        partnershipDeed: partnershipDeed?.docPath || "",
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
      ...
      const partnershipDeed = existingDocuments.find(
        (d) => d.docType === DocType.PartnershipDeed,
      );
      const taxReg = existingDocuments.find(
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel required>Partnership Deed</FormLabel>
              <FormControl>
                <UploadField
                  {...field}
                  type="identity"
                  ...
                  supportingText="Upload your partnership deed (PDF, JPEG, PNG up to 10MB)"
                />
              </FormControl>
              <FormMessage />
            </FormItem>
